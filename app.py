...
class Todo(db.Model):
    ...
    def __repr__(self):
        return f'<Todo id={self.id} title={self.title} done={self.done} created_at={self.created_at} favorited={self.favorited}>'

# Create the database and table to hold the games
with app.app_context():
    db.create_all()

# Create a home route that displays the game list
@app.route('/')
def index():
...
# Create a route to delete a game
...
# Create a route to favorite a game
@app.route('/favorite/<int:todo_id>')
def favorite(todo_id):
    todo = Todo.query.get_or_404(todo_id)
    todo.favorited = not todo.favorited
    db.session.commit()
    return redirect(url_for('index'))
